...
def write_binary_stl(path: str, triangles: List[Tuple[Tuple[float, float, float], Tuple[float, float, float], Tuple[float, float, float]]]):
    header = b"pb3 seam engine solid/perforated dome modes".ljust(80, b"\0")
...
class LampParams:
    ...
    seam_width: float = 4.0
    seam_height: float = 1.6
    seam_softness: float = 2.2
    valley_depth: float = 0.55
    ...
    # perforated mode ribs
    rib_width_scale: float = 0.72
    rib_thickness: float = 1.2
    rib_seg_per_pitch: int = 90

    # dome
    dome_height_scale: float = 0.42  # 0.28 chato, 0.42 normal, 0.60 alto
...
def add_top_cap_dome_solid(tris, p: LampParams, z_base: float):
    """
    Domo sólido completo.
    """
    n_rings = 28
    n_theta = p.n_theta

    r_base = radius_profile(z_base, p) + p.seam_height * 0.35
    dome_height = r_base * p.dome_height_scale
    dome_height = max(1.0, dome_height)

    sphere_R = (r_base * r_base + dome_height * dome_height) / (2.0 * dome_height)
    sphere_center_z = z_base + dome_height - sphere_R

    rings = []

    for ir in range(n_rings + 1):
        t = ir / n_rings
        z = z_base + dome_height * t
        rr = math.sqrt(max(0.0, sphere_R * sphere_R - (z - sphere_center_z) * (z - sphere_center_z)))

        ring = []
        for it in range(n_theta):
            theta = (2.0 * math.pi * it) / n_theta
            ring.append(polar(rr, theta, z))
        rings.append(ring)
    ...
    # unir cuerpo a base del domo
    body_ring = []
    dome_ring = rings[0]

    for it in range(n_theta):
        theta = (2.0 * math.pi * it) / n_theta
        r_body = radius_profile(z_base, p) + p.seam_height * 0.35
        body_ring.append(polar(r_body, theta, z_base))
    ...
    top_point = (0.0, 0.0, z_base + dome_height)
    last_ring = rings[-1]

    for it in range(n_theta):
        it2 = (it + 1) % n_theta
        tris.append((last_ring[it], last_ring[it2], top_point))


def add_top_cap_dome_perforated(tris, p: LampParams, z_base: float):
    """
    Domo perforado, armado por ribs/meridianos.
    """
    n_rings = 28
    n_theta = p.n_theta
    ...
    rib_width = max(0.5, p.seam_width * p.rib_width_scale)
    dome_ribs = max(8, p.seam_count)

    for k in range(dome_ribs):
        theta_c = (2.0 * math.pi * k) / dome_ribs
        ...
        for ir in range(n_rings + 1):
            ...
            dth = rib_width / max(p.r_min, rr)
            th_a = theta_c - dth * 0.5
            th_b = theta_c + dth * 0.5

            ro = rr
            ri = max(p.r_min, rr - p.rib_thickness)

            outer_a = polar(ro, th_a, z)
            outer_b = polar(ro, th_b, z)
            inner_a = polar(ri, th_a, z)
            inner_b = polar(ri, th_b, z)

            if prev_outer_a is not None:
                add_quad(tris, prev_outer_a, prev_outer_b, outer_b, outer_a, flip=False)
                add_quad(tris, prev_inner_a, inner_a, inner_b, prev_inner_b, flip=False)
                add_quad(tris, prev_outer_a, outer_a, inner_a, prev_inner_a, flip=False)
                add_quad(tris, prev_outer_b, prev_inner_b, inner_b, outer_b, flip=False)

            prev_outer_a = outer_a
            prev_outer_b = outer_b
            prev_inner_a = inner_a
            prev_inner_b = inner_b

    # anillo de unión con el cuerpo
    nt = dome_ribs
    body_outer = []
    dome_outer = []

    for k in range(nt):
        theta = (2.0 * math.pi * k) / nt
        r_body = radius_profile(z_base, p) + p.seam_height * 0.35
        body_outer.append(polar(r_body, theta, z_base))
        dome_outer.append(polar(r_base, theta, z_base))

    for k in range(nt):
        k2 = (k + 1) % nt
        add_quad(tris, body_outer[k], body_outer[k2], dome_outer[k2], dome_outer[k], flip=False)


# =========================================================
# Perforated mode mesh
# =========================================================

def make_mesh_perforated(p: LampParams, close_top: bool = False, dome_mode: str = "solid"):
    tris = []
    ...
    if close_top:
        if dome_mode == "solid":
            add_top_cap_dome_solid(tris, p, p.height)
        else:
            add_top_cap_dome_perforated(tris, p, p.height)
...
def main():
    ap = argparse.ArgumentParser(description="PB3 seam engine with solid/perforated modes and solid/perforated dome")
    ...
    ap.add_argument("--close-top", action="store_true", help="Close the top opening with a dome in perforated mode")
    ap.add_argument("--dome-mode", choices=["solid", "perforated"], default="solid", help="Dome type when close-top is used")
    ...
    # dome
    ap.add_argument("--dome-height-scale", type=float, default=0.42, help="0.28 flat, 0.42 normal, 0.60 tall")
    ...
    for i in range(args.count):
        p = LampParams(
            ...
            dome_height_scale=args.dome_height_scale,
        ...
        if args.mode == "solid":
            ...
            tris = make_mesh_perforated(
                p,
                close_top=args.close_top,
                dome_mode=args.dome_mode
            )
