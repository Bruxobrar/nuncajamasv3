...
def write_binary_stl(
    ...
    header = b"lampgen_v6_cube_closures".ljust(80, b"\0")
...
def sanitize_params(p: LampParams) -> LampParams:
    return LampParams(
        ...
        top_style=clamp(p.top_style, 0.0, 3.0),
...
def texture_fade(u: float) -> float:
    bottom = smoothstep(min(u / 0.08, 1.0))
    top = smoothstep(min((1.0 - u) / 0.14, 1.0))
    return bottom * top
...
def shaped_radius(theta: float, z: float, p: LampParams) -> float:
    u = z / p.height
    square_factor = square_radius_factor(theta, p)
    shaped = radius_profile(z, p) * square_factor
    landing = max(shaped, p.r_base * 1.06 * square_factor)
    bottom_blend = 1.0 - smoothstep(min(u / 0.07, 1.0))
    return shaped * (1.0 - bottom_blend) + landing * bottom_blend
...
def top_boundary_point(theta: float, p: LampParams):
    z_top = p.height
    base_radius = shaped_radius(theta, z_top, p)
    disp = weave_displacement(theta, z_top, p, base_radius)
    outer_disp = disp * (1.0 - p.outer_smoothing)
    return polar(max(p.r_min + p.thickness + 0.3, base_radius + outer_disp), theta, z_top)


def add_flat_cap(tris, ring):
    center = (0.0, 0.0, ring[0][2])
    count = len(ring)
    for index in range(count):
        next_index = (index + 1) % count
        tris.append((ring[index], ring[next_index], center))


def add_dome_cap(tris, ring, height_scale):
    count = len(ring)
    z_top = ring[0][2]
    max_radius = max(math.hypot(point[0], point[1]) for point in ring)
    dome_height = max_radius * height_scale
    rings = [ring]

    for step in range(1, 8):
        t = step / 7.0
        shrink = math.sqrt(max(0.0, 1.0 - t * t))
        z = z_top + dome_height * t
        next_ring = []
        for point in ring:
            next_ring.append((point[0] * shrink, point[1] * shrink, z))
    ...
    for ridx in range(len(rings) - 1):
        ...
        for index in range(count):
            next_index = (index + 1) % count
            add_quad(tris, current[index], current[next_index], nxt[next_index], nxt[index], flip=False)

    tip = (0.0, 0.0, z_top + dome_height)
    last = rings[-1]
    for index in range(count):
        next_index = (index + 1) % count
        tris.append((last[index], last[next_index], tip))


def add_pyramid_cap(tris, ring, height_scale):
    count = len(ring)
    z_top = ring[0][2]
    max_radius = max(math.hypot(point[0], point[1]) for point in ring)
    tip = (0.0, 0.0, z_top + max_radius * height_scale)
    for index in range(count):
        next_index = (index + 1) % count
        tris.append((ring[index], ring[next_index], tip))


def add_inset_cap(tris, ring, height_scale):
    count = len(ring)
    z_top = ring[0][2]
    max_radius = max(math.hypot(point[0], point[1]) for point in ring)
    inset_ring = []
    for point in ring:
        inset_ring.append((point[0] * 0.28, point[1] * 0.28, z_top + max_radius * height_scale * 0.65))

    for index in range(count):
        next_index = (index + 1) % count
        add_quad(tris, ring[index], ring[next_index], inset_ring[next_index], inset_ring[index], flip=False)

    add_flat_cap(tris, inset_ring)


def add_top_closure(tris, p: LampParams):
    ring = [top_boundary_point((2.0 * math.pi * index) / p.n_theta, p) for index in range(p.n_theta)]
    style = int(round(p.top_style))
    if style <= 0:
        add_flat_cap(tris, ring)
    elif style == 1:
        add_dome_cap(tris, ring, 0.24)
    elif style == 2:
        add_pyramid_cap(tris, ring, 0.30)
    else:
        add_inset_cap(tris, ring, 0.22)


def make_mesh_solid(params: LampParams, close_top: bool = False):
    ...
    for iz in range(p.n_z):
        ...
        for it in range(p.n_theta):
            ...
            top_finish = smoothstep(max(0.0, (u - 0.82) / 0.18))
            outer_smoothing = clamp(p.outer_smoothing + 0.55 * top_finish, 0.0, 1.0)
            inner_smoothing = clamp(p.inner_smoothing + 0.20 * top_finish, 0.0, 1.0)
            shell_thickness = p.thickness + 0.45 * top_finish
            outer_disp = raw_disp * (1.0 - outer_smoothing)
            inner_disp = raw_disp * p.inner_follow * (1.0 - inner_smoothing)

            outer_radius = max(p.r_min + shell_thickness + 0.3, base_radius + outer_disp)
            inner_radius = max(p.r_min, base_radius - shell_thickness + inner_disp)
...
def make_mesh_perforated(params: LampParams, close_top: bool = False):
    p = sanitize_params(params)
    tris = []
...
def main():
    ap = argparse.ArgumentParser(description="Lampgen V6 cube -> STL (cube shell + closures)")
    ...
    ap.add_argument("--close-top", action="store_true")
    ...
    ap.add_argument("--top-style", type=float, default=0.0)
    ...
    for i in range(args.count):
        ...
        tris = make_mesh_perforated(p, close_top=args.close_top) if args.mode == "perforated" else make_mesh_solid(p, close_top=args.close_top)
        name = (
            f"lampgenv6_{args.mode}_h{int(p.height)}_rb{int(p.r_base)}_"
            f"cm{p.cube_mix:.2f}_os{p.outer_smoothing:.2f}_is{p.inner_smoothing:.2f}_ts{p.top_style:.0f}_"
            f"{i:04d}.stl"
        )
