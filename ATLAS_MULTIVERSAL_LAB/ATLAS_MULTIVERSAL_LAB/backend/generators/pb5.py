...
def write_binary_stl(path: str, triangles: List[Tuple[Tuple[float, float, float], Tuple[float, float, float], Tuple[float, float, float]]]):
    header = b"pb3 seam engine solid/perforated dome-top".ljust(80, b"\0")
...
def add_top_cap_dome(tris, p: LampParams, z_base: float):
    """
    Close the top with a shallow dome.
    z_base = top height of the lamp body.
    """
    n_rings = 28
    n_theta = p.n_theta
    ...
    # bridge body top to dome base ring
    body_ring = []
    dome_ring = rings[0]
...
def make_mesh_perforated(p: LampParams, close_top: bool = False):
    tris = []
    ...
    if close_top:
        add_top_cap_dome(tris, p, p.height)
...
def main():
    ap = argparse.ArgumentParser(description="PB3 seam engine with solid/perforated modes and dome top")
    ...
    for i in range(args.count):
        ...
        if args.mode == "solid":
            ...
            tris = make_mesh_perforated(p, close_top=args.close_top)
