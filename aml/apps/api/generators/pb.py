...
def write_binary_stl(path: str, triangles: List[Tuple[Tuple[float, float, float], Tuple[float, float, float], Tuple[float, float, float]]]):
    header = b"lampgen_v1_continuous_seam".ljust(80, b"\0")
...
def weave_displacement(theta: float, z: float, p: LampParams) -> float:
    """
    Costura diagonal continua:
    - una helicoidal principal dominante
    - una secundaria suave para dar cuerpo
    - seam killer muy leve
    """
    H = p.height
    u = z / H

    # seam killer MUY suave
    layer_twist = 0.01 * math.sin(u * math.pi)
    theta = theta + layer_twist

    # helicoidal principal
    main = math.sin(p.weave_theta * theta + (p.weave_pitch * 2.0 * math.pi) * u)

    # helicoidal secundaria más leve
    secondary = math.sin(-p.weave_theta * theta + (p.weave_pitch * 2.0 * math.pi) * u)

    # dominante la principal para que se lea la costura
    s = 0.78 * main + 0.22 * secondary
...
def make_mesh(p: LampParams):
    ...
    - Inner surface: same deformation offset inward by thickness
    ...
    for iz in range(nz):
        ...
        for it in range(nt):
            ...
            # espesor más uniforme para mejor translucidez
...
def main():
    ap = argparse.ArgumentParser(description="Procedural lamp head generator V1 -> STL (continuous seam)")
