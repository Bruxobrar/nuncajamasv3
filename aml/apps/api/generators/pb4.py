...
def write_binary_stl(path: str, triangles: List[Tuple[Tuple[float, float, float], Tuple[float, float, float], Tuple[float, float, float]]]):
    header = b"pb3 seam engine solid/perforated close-top".ljust(80, b"\0")
...
def add_top_cap_ring(tris, p: LampParams, z_top: float):
    """
    Close the top with a ring/disc and a short downward skirt.
    Useful in perforated mode so the mouth doesn't stay open.
    """
    nt = p.n_theta
    r_outer = radius_profile(z_top, p) + p.seam_height * 0.35
    ring_depth = max(p.thickness, p.rib_thickness)
    r_inner = max(p.r_min, r_outer - ring_depth)
    ...
    for it in range(nt):
        theta = (2.0 * math.pi * it) / nt
        outer.append(polar(r_outer, theta, z_top))
        inner.append(polar(r_inner, theta, z_top))
    ...
    for it in range(nt):
        ...
        # short vertical skirt
        z2 = z_top - max(1.2, p.rib_thickness)
        o0d = (o0[0], o0[1], z2)
        o1d = (o1[0], o1[1], z2)
        i0d = (i0[0], i0[1], z2)
        i1d = (i1[0], i1[1], z2)

        add_quad(tris, o0, o1, o1d, o0d, flip=False)
        add_quad(tris, i0, i0d, i1d, i1, flip=False)
...
def make_mesh_perforated(p: LampParams, close_top: bool = False):
    ...
    # optional thin membrane backing
    if p.membrane > 0.001:
    ...
    if close_top:
        add_top_cap_ring(tris, p, p.height)
...
def main():
    ...
    ap.add_argument("--close-top", action="store_true", help="Close the top opening in perforated mode")
