...
const BASE_RELEVANT_KEYS = new Set([
  "socket_id",
  "n_lugs",
  "twist_deg",
  "entry_deg",
...
async function requestPreview() {
  ...
  try {
    ...
    state.previewMeta = data.meta || null;
    state.previewSource = "generated";
    const targetNote = data.meta?.target_triangles ? ` / target ${data.meta.target_triangles}` : "";
    const previewNote = data.meta?.preview_note ? ` | ${data.meta.preview_note}` : "";
    const engineLabel = state.engineConfig?.label ?? state.engineId;
    meshStats.textContent = `${engineLabel} | ${data.mesh.vertex_count} vertices | ${data.mesh.triangle_count} triangles${targetNote} | preview${previewNote}`;
...
const debouncedBases = debounce(refreshBaseRecommendations, 200);

function baseElements() {
  return {
    panel: document.getElementById("basePanel"),
    interfaceLine: document.getElementById("baseInterfaceLine"),
    footprintLine: document.getElementById("baseFootprintLine"),
    familySelect: document.getElementById("baseFamilySelect"),
    select: document.getElementById("baseSelect"),
    applyBtn: document.getElementById("applyBaseBtn"),
    confirmBtn: document.getElementById("confirmBaseFlowBtn"),
    notes: document.getElementById("baseNotes"),
  };
}

function getSelectedBaseId() {
  const { select } = baseElements();
  if (!select) {
    return null;
  }
...
function findBaseInCatalog(baseId) {
  const catalog = state.baseCatalog?.bases ?? [];
  return catalog.find((item) => item.id === baseId) ?? null;
}

function describeMount(mount) {
  if (!mount) {
    return "Interface: (sin datos)";
  }
  const source = mount.source ? ` | fuente ${mount.source}` : "";
  if (mount.type === "bayonet_female" && typeof mount.female_id_mm === "number") {
    return `Interface: bayonet hembra | ID ${mount.female_id_mm.toFixed(1)} mm${source}`;
  }
  if (mount.type === "ring" && typeof mount.female_id_mm === "number") {
    return `Interface: aro / plug | ID ${mount.female_id_mm.toFixed(1)} mm${source}`;
  }
  return `Interface: ${mount.type || "unknown"}${source}`;
}

function renderBasePanel() {
  const { panel, interfaceLine, footprintLine, familySelect, select, applyBtn, notes } = baseElements();
  if (!panel || !interfaceLine || !footprintLine || !familySelect || !select || !applyBtn || !notes) {
    return;
  }

  interfaceLine.textContent = describeMount(state.mountInterface);
  if (state.baseFootprint?.sample_count > 0) {
    footprintLine.textContent = `Huella boca: avg ${Number(state.baseFootprint.diameter_avg || 0).toFixed(1)} mm | p90 ${Number((state.baseFootprint.radius_p90 || 0) * 2).toFixed(1)} mm`;
  } else {
    footprintLine.textContent = "";
  }

  familySelect.innerHTML = "";
  const families = state.baseFamilies ?? [];
  if (!families.length) {
    const familyOption = document.createElement("option");
    familyOption.value = "";
    familyOption.textContent = "Sin tipos disponibles";
    ...
    for (const family of families) {
      ...
      const compatibility = family.compatible ? "" : " no compatible";
      const recommendation = family.recommended ? " recomendado" : "";
      familyOption.textContent = `${family.label}${compatibility}${recommendation}`;
      familySelect.appendChild(familyOption);
    }
    familySelect.value = state.selectedBaseFamily ?? families[0].id;
    familySelect.disabled = false;
  }

  const recs = state.baseRecommendations ?? [];
  select.innerHTML = "";
  if (!recs.length) {
    const option = document.createElement("option");
    option.value = "";
    option.textContent = "Sin variantes para este modelo";
    select.appendChild(option);
    select.disabled = true;
    applyBtn.disabled = true;
    notes.textContent = "Tip: si el motor soporta encastre, ajustá `socket_id` / `opening_radius` y reintenta.";
    return;
  }
  ...
  for (const rec of recs) {
    const option = document.createElement("option");
    option.value = rec.id;
    option.textContent = `${rec.label}${typeof rec.score === "number" ? ` (score ${rec.score.toFixed(2)})` : ""}`;
    select.appendChild(option);
  }
  ...
  const selectedId = getSelectedBaseId() ?? recs[0].id;
  select.value = selectedId;
  const selected = findBaseInCatalog(selectedId);
  const selectedRec = recs.find((r) => r.id === selectedId) ?? null;
  const hint = selectedRec?.reason ? `\n${selectedRec.reason}` : "";
  const familyLabel = selectedRec?.family_label ? `${selectedRec.family_label}\n` : "";
  notes.textContent = `${familyLabel}${selected?.notes ?? ""}${hint}`.trim();
}

function applyBaseProfile(baseId) {
  const base = findBaseInCatalog(baseId);
  if (!base?.mount_profile) {
    statusLine.textContent = "No base profile available.";
    return;
  }
  let changed = false;
  for (const [key, value] of Object.entries(base.mount_profile)) {
    if (!(key in state.engineConfig.defaults)) {
      continue;
    }
    state.params[key] = sanitizeNumericParam(key, value);
    changed = true;
  }

  if (!changed) {
    statusLine.textContent = "Base profile not compatible with this engine.";
    return;
  }

  statusLine.textContent = `Applied base profile: ${base.label}`;
  buildControls();
  debouncedPreview();
  debouncedBases();
}

function goToBaseStudio() {
  const selection = {
    engine: state.engineId,
    engineLabel: state.engineConfig?.label ?? state.engineId,
    preset: state.currentPreset,
    model: collectPayload(),
    mountInterface: state.mountInterface,
    baseFamily: state.selectedBaseFamily,
    recommendedBaseId: getSelectedBaseId(),
    createdAt: new Date().toISOString(),
  };
  const ok = window.confirm("Confirmar este modelo y pasar al editor de base?");
  if (!ok) {
    return;
  }
  sessionStorage.setItem("amlSelectedLampModel", JSON.stringify(selection));
  window.location.href = "/base-studio/";
}

async function ensureBaseCatalogLoaded() {
  if (state.baseCatalog) {
    return;
  }
  try {
    state.baseCatalog = await fetchJson("/api/bases/catalog");
  } catch (error) {
    state.baseCatalog = { bases: [] };
    const { interfaceLine } = baseElements();
    if (interfaceLine) {
      interfaceLine.textContent = `Interface: (catalog error: ${error.message})`;
    }
  }
}

async function refreshBaseRecommendations() {
  const { interfaceLine } = baseElements();
  await ensureBaseCatalogLoaded();
  if (!state.baseCatalog?.bases?.length) {
    state.baseRecommendations = [];
    state.mountInterface = { type: "unknown" };
    state.baseFamilies = [];
    state.baseFootprint = null;
    renderBasePanel();
    return;
  }

  try {
    const data = await fetchJson("/api/bases/recommend", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...collectPayload(),
        base_family: state.selectedBaseFamily,
      }),
    });
    state.baseFamilies = data.base_families ?? [];
    state.selectedBaseFamily = data.selected_family ?? state.selectedBaseFamily;
    state.baseRecommendations = data.recommendations ?? [];
    state.mountInterface = data.mount_interface ?? { type: "unknown" };
    state.baseFootprint = data.footprint ?? null;
    renderBasePanel();
  } catch (error) {
    state.baseRecommendations = [];
    state.mountInterface = { type: "unknown" };
    ...
    state.baseFootprint = null;
    if (interfaceLine) {
      interfaceLine.textContent = `Interface: (error: ${error.message})`;
    }
    renderBasePanel();
  }
}
...
function draw() {
  ...
  const renderBackfaces = state.previewMeta?.engine_kind === "scad";

  for (const face of faces) {
    if (!renderBackfaces && face.facing <= 0) {
      continue;
    }
...
async function init() {
  ...
  const requestedPreset = query.get("preset") || "default";

  const engineSelect = document.getElementById("engineSelect");
  engineSelect.innerHTML = "";
  const familyLabels = { lamp: "LampGen", pb: "PB", lgb: "LGB", design: "Design", misc: "Other" };
  const familyGroups = new Map();
  for (const engine of state.config.engines) {
    const family = engine.family ?? "misc";
    if (!familyGroups.has(family)) {
      const group = document.createElement("optgroup");
      group.label = familyLabels[family] ?? family.toUpperCase();
      familyGroups.set(family, group);
      engineSelect.appendChild(group);
    }
    const option = document.createElement("option");
    option.value = engine.id;
    option.textContent = engine.label;
    familyGroups.get(family).appendChild(option);
  }
  ...
  const baseFamilySelect = document.getElementById("baseFamilySelect");
  baseFamilySelect?.addEventListener("change", () => {
    state.selectedBaseFamily = baseFamilySelect.value || null;
    refreshBaseRecommendations();
  });
  baseSelect?.addEventListener("change", () => renderBasePanel());
  document.getElementById("applyBaseBtn")?.addEventListener("click", () => {
    const baseId = getSelectedBaseId();
    if (baseId) {
      applyBaseProfile(baseId);
    }
  });
  document.getElementById("confirmBaseFlowBtn")?.addEventListener("click", goToBaseStudio);
  ...
  const availableEngines = new Set(Object.keys(state.config.engine_configs || {}));
  const initialEngine = requestedEngine && availableEngines.has(requestedEngine)
    ? requestedEngine
    : state.config.default_engine;
  engineSelect.value = initialEngine;
  loadEngine(initialEngine, requestedPreset);
  await ensureBaseCatalogLoaded();
  await refreshBaseRecommendations();
  await refreshExports();
