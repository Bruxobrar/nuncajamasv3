...
function clonePreset(name) {
  return JSON.parse(JSON.stringify(state.config.presets[name]));
}
...
function sanitizeNumericParam(key, value) {
  const fallback = state.config.defaults[key];
...
function getParamDescription(key) {
  return state.config.descriptions?.[key] ?? PARAM_TOOLTIPS[key] ?? key;
}

function applyPayloadToState(payload) {
  const nextParams = {};
  for (const key of Object.keys(state.config.defaults)) {
    nextParams[key] = sanitizeNumericParam(key, payload[key] ?? state.config.defaults[key]);
  }

  state.params = nextParams;
  document.getElementById("modeSelect").value =
    payload.mode === "perforated" ? "perforated" : "solid";
  document.getElementById("closeTopInput").checked = Boolean(payload.close_top);
  document.getElementById("domeModeSelect").value =
    payload.dome_mode === "perforated" ? "perforated" : "solid";
}
...
function collectPayload() {
  return {
    ...
    close_top: document.getElementById("closeTopInput").checked,
    dome_mode: document.getElementById("domeModeSelect").value,
  };
}
...
async function requestPreview() {
  ...
  try {
    ...
    meshStats.textContent = `${data.mesh.vertex_count} vertices | ${data.mesh.triangle_count} triangles | preview`;
...
async function importModelFile(file) {
  try {
    ...
    buildControls();
    statusLine.textContent = `Imported ${data.file}. Sliders still control PB6 preview when you move them.`;
    draw();
  } catch (error) {
...
async function init() {
  ...
  const presetSelect = document.getElementById("presetSelect");
  for (const presetName of Object.keys(state.config.presets)) {
